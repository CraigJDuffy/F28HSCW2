...
#define	LCD_CDSHIFT	0x10 // Base bit sequence for Cursor or Display Shift
#define	LCD_FUNC	0x20 // Base bit sequence for Function Set instruction
...
#define	LCD_FUNC_F	0x04 // Sets font to 1: 5*10 dot
#define	LCD_FUNC_N	0x08 // Sets Number of diplay lines to 1: 2 Lines
#define	LCD_FUNC_DL	0x10 // Sets Data Length to 1: 8-bit mode
...
void lcdClear (struct lcdDataStruct *lcd)
{
 ...
 // lcdPutCommand (lcd, LCD_HOME) ; //If you read the documentation, you'd know that LCD_CLEAR instruction also excutes LCD_HOME automatically.
  lcd->cx = lcd->cy = 0 ;
  delay (5) ;
}
...
void lcdPutsScroll (struct lcdDataStruct * lcd, const char *string){
	lcdPutCommand(lcd, LCD_ENTRY | LCD_ENTRY_SH | LCD_ENTRY_ID); //Set to shift mode
	lcdPutCommand(lcd, LCD_DGRAM | 0x10);
	while (*string){
	digitalWrite (gpio, lcd->rsPin, 1) ;
	sendDataCmd  (lcd, *string++) ;
	usleep(250000);
	}
	lcdPutCommand(lcd, LCD_ENTRY | LCD_ENTRY_ID) ;//Set back to cursor move mode
}
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  //while(1){
	  ...
	  //sprintf(stringTime, "%d:%d:%d", localTime->tm_hour, localTime->tm_min, localTime->tm_sec);
	  //lcdPuts(lcd, stringTime);
	  //usleep(990000);
	  //lcdClear(lcd);
  //}
  
  //lcdPutsScroll(lcd, "A Nice Long String Literal");
